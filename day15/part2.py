...
class Unit():
    def __init__(self, x, y, elf, attackpower):
        self.x = x
        self.y = y
        self.hitpoints = 200
        self.elf = elf
...
while elves_die:
    ...
    for y, line in enumerate(lines):
        ...
        for x, c in enumerate(line.strip()):
            if c == 'G':
                u = Unit(x, y, elf=False, attackpower=3)
            elif c == 'E':
                u = Unit(x, y, elf=True, attackpower=elf_power)
            elif c == '#':
                u = '#'
            elif c.strip() == '':
                break
    ...
    rounds = 1
    while not done and not elves_die:
        ...
        for idx, unit in enumerate(turn_order):
            ...
            rounds += 1
    elf_power += 1
    print(elf_power, done, elves_die, rounds)
